...
def transform_to_log_coordinates(x):
    if not isinstance(x, torch.Tensor):
        x = torch.tensor(x)
    log_x = (torch.log10(torch.where(x <= 0, torch.finfo(torch.float).eps, x)) - torch.log10(torch.tensor(0.1))) / (torch.log10(torch.tensor(1.0)) - torch.log10(torch.tensor(0.1))) # 将负值和零转换为正值,取对数,归一化处理 现在是0.1-1而不是0-8了~！
    return log_x
# def transform_to_log_coordinates(x):
#     if not isinstance(x, torch.Tensor):
#         x = torch.tensor(x)
...
class EDataset(Dataset.Dataset):
    def __init__(self, rcsdir):    #初始化，定义数据内容和标签
        self.rcsdir = rcsdir
        self.filelist = os.listdir(rcsdir)
...
def load_and_process(file_path):
    ...
    if os.path.exists(pt_path):
        faceedge = torch.load(pt_path,weights_only=False)
    else:
        # faceedge = pt_path
        print('正在生成edge')
